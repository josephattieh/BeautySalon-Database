...
*/
	...
	/*public void searchCustomer(Stage primaryStage, Scene3 scene3) {
		try {
			getButton().setOnAction(e -> {

				pane.getChildren().add(searchname);
				searchname.setOnKeyPressed(event -> {
					if (event.getCode() == KeyCode.ENTER) {
						buildData("SELECT * FROM STAFF WHERE fName=\"" + getSearchname().getText() + "\";", tableview);
						searchname.setText("");
					}
				});
	...
	public void refresh () {
		button1.setOnAction(e->{
			...
			buildData("SELECT * FROM CUSTOMER;",getTableView());
		});
	}
